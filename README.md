...
![Server Manager initial state](DC1 LAB/01_server-manager-initial.png)
...
![Static IP configuration](DC1 LAB/02_static-ip-configuration.png)

Verified network connectivity from the client machine with a successful ping to the DC — 4 packets sent, 0% packet loss.

![Ping verification from client](DC1 LAB/03_ping-verification.png)
...
![Selecting AD DS and DNS roles](DC1 LAB/04_role-installation-adds-dns.png)
...
![Deployment configuration - new forest](DC1 LAB/05_dc-promotion-new-forest.png)
...
![Domain Controller options](DC1 LAB/06_dc-options-functional-level.png)

Acknowledged the DNS delegation warning (expected for a new isolated lab forest) and left DNS delegation unchecked.

![DNS Options](DC1 LAB/07_dns-options.png)

NetBIOS name auto-resolved to `LAB`.

![NetBIOS name](DC1 LAB/08_netbios-name.png)

Confirmed default AD DS database, log, and SYSVOL paths under `C:\Windows\`.

![AD DS paths](DC1 LAB/09_ad-ds-paths.png)

Reviewed final configuration summary before install.

![Review options](DC1 LAB/10_review-options.png)

All prerequisite checks passed successfully.

![Prerequisites check passed](DC1 LAB/11_prerequisites-check-passed.png)

After reboot, Server Manager confirmed **AD DS** and **DNS** roles both active and healthy (Roles: 3).

![Server Manager post-promotion with AD DS and DNS](DC1 LAB/12_server-manager-adds-dns-active.png)
...
![ADUC - lab.local domain](DC1 LAB/13_aduc-lab-local-domain.png)
...
![OU structure - all departments](DC1 LAB/14_ou-structure.png)
...
![IT_Group security group](DC1 LAB/15_it-group-security-group.png)

**HR_Group** in the HR OU:

![HR_Group security group](DC1 LAB/16_hr-group-security-group.png)

**Finance_Group** in the Finance OU:

![Finance_Group security group](DC1 LAB/17_finance-group-security-group.png)
...
![New user - Alice Doe details](DC1 LAB/18_new-user-alice-doe.png)

Set account password and configured password options.

![New user - password setup](DC1 LAB/19_new-user-password-setup.png)

Confirmed user creation summary before finishing.

![New user - confirmation](DC1 LAB/20_new-user-confirmation.png)

Added Alice Doe to `HR_Group` — confirmed via the group's Members tab.

![HR_Group members - Alice Doe](DC1 LAB/21_hr-group-members-alice-doe.png)
...
![Group Policy Management console](DC1 LAB/22_group-policy-management-console.png)

Created a new GPO named **`HR_Restrictions`** linked to the HR OU.

![New GPO - HR_Restrictions](DC1 LAB/23_new-gpo-hr-restrictions.png)
...
![GPO editor - Control Panel policy](DC1 LAB/24_gpo-editor-control-panel-policy.png)

Enabled the policy to block all Control Panel and PC Settings access for HR users.

![Policy enabled](DC1 LAB/25_gpo-policy-enabled.png)
...
![Domain join - entering lab.local](DC1 LAB/26_domain-join-lab-local.png)

Authenticated with domain admin credentials to complete the join.

![Domain join - authentication](DC1 LAB/27_domain-join-authentication.png)

After restart, the full device name resolved to `DESKTOP-15Q3OG7.lab.local` confirming successful domain membership.

![Domain joined - full device name confirmed](DC1 LAB/28_domain-joined-confirmed.png)
...
![Login screen - LAB\adoe](DC1 LAB/29_login-lab-adoe.png)

Searched for **Control Panel** on the domain-joined machine.

![Searching for Control Panel](DC1 LAB/30_searching-control-panel.png)
...
![GPO enforcement - restriction error](DC1 LAB/31_gpo-restriction-error.png)
